...
import { usdcABI as abi, authorizationTypes, config, } from "../../../types/shared/evm";
 ...
 */
export async function verify(client, payload, paymentRequirements) {
    ...
    try {
        ...
        console.log(`[DEBUG] getNetworkId("${payload.network}") = ${chainId}`);
        console.log(`[DEBUG] config keys:`, Object.keys(config));
        console.log(`[DEBUG] config[${chainId}]:`, config[chainId.toString()]);
        ...
        console.error(`[DEBUG] Error in network config:`, error);
        return {
            isValid: false,
    ...
    if (BigInt(exactEvmPayload.authorization.validBefore) < BigInt(Math.floor(Date.now() / 1000) + 6)) {
        return {
            isValid: false,
    ...
    // Verify client has enough funds to cover paymentRequirements.maxAmountRequired
    const balance = await getERC20Balance(client, erc20Address, exactEvmPayload.authorization.from);
 ...
 */
export async function settle(wallet, paymentPayload, paymentRequirements) {
    ...
    // Split signature into v, r, s components for contracts that require separate parameters
    // Signature format: 0x + r(64 chars) + s(64 chars) + v(2 chars) = 132 chars total
    const r = signature.slice(0, 66); // 0x + 64 chars
    const s = ('0x' + signature.slice(66, 130)); // 0x + 64 chars
