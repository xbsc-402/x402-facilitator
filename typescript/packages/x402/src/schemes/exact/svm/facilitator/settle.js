import { assertIsTransactionMessageWithBlockhashLifetime, decompileTransactionMessageFetchingLookupTables, getBase64EncodedWireTransaction, getCompiledTransactionMessageDecoder, getSignatureFromTransaction, isSolanaError, signTransaction, SOLANA_ERROR__BLOCK_HEIGHT_EXCEEDED, } from "@solana/kit";
 ...
 */
export async function settle(signer, payload, paymentRequirements, config) {
    ...
    const rpcSubscriptions = getRpcSubscriptions(paymentRequirements.network, config?.svmConfig?.rpcUrl);
    try {
        const { success, errorReason, signature } = await sendAndConfirmSignedTransaction(signedTransaction, rpc, rpcSubscriptions);
        return {
            success,
            errorReason,
            payer,
 ...
 */
export async function sendSignedTransaction(signedTransaction, rpc, sendTxConfig = {
    skipPreflight: true,
    encoding: "base64",
}) {
 ...
 */
export async function confirmSignedTransaction(signedTransaction, rpc, rpcSubscriptions) {
 ...
 * @returns The success and signature of the confirmed transaction
 */
export async function sendAndConfirmSignedTransaction(signedTransaction, rpc, rpcSubscriptions) {
