import { createSolanaRpc, devnet, mainnet, createSolanaRpcSubscriptions, } from "@solana/kit";
/**
 ...
 */
export function createDevnetRpcClient(url) {
    return createSolanaRpc(url ? devnet(url) : devnet(DEVNET_RPC_URL));
}
/**
 ...
 * @returns A Solana RPC client.
 */
export function createMainnetRpcClient(url) {
    return createSolanaRpc(url ? mainnet(url) : mainnet(MAINNET_RPC_URL));
}
/**
 ...
 */
export function getRpcClient(network, url) {
 ...
 */
export function getRpcSubscriptions(network, url) {
    // TODO: should the networks be replaced with enum references?
    if (network === "solana-devnet") {
